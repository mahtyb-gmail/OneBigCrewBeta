namespace :db do
  ...
  task populate: :environment do
    20.times do |n|
      ...
      puts "[DEBUG] creating bio for user with user_id = #{user_id}"
        skillset = (1..29).to_a
        fourand = skillset.sort{ rand() - 0.5}[ 0..3]
        4.times do |x|
                skill_id = fourand[x]
                bio = Bio.create!( user_id: user_id, skill_id: skill_id )
        end
    end
	    ...
	    30.times do |n|
	        ...
	        4.times do |x|
	                skill_id = fourand[x]
	                req = Req.create!( project_id: project.id, skill_id: skill_id )
	      end
	    end
	  end
	end